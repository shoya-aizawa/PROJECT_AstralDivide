...
rem -----------------------------------------------------------------------------
rem ScreenEnvironmentDetection.bat  (RCSU/RC 統合版)
rem Role:
...
rem === ログ & ESC ==============================================================
set "LOG_PREFIX=env"
call "%RCSU%" -trace INFO ScreenEnv "start cfg=%screen_cfg_file%"
...
if "%HAS_CMDWIZ%"=="0" call "%RCSU%" -trace WARN ScreenEnv "cmdwiz.exe not found at %tools_dir% (will degrade)"
...
if "%HAS_CMDWIZ%"=="1" (
  ...
  call "%RCSU%" -trace INFO ScreenEnv "fullscreen=on rc=%errorlevel%"
...
if defined BAD (
  call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_PARSE% 011 "screen size parse failed" "W=%screen_width% H=%screen_height%"
)
...
if defined BAD (
  call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_PARSE% 011 "console size parse failed" "W=%console_width% H=%console_height%"
)
...
if "%console_width%"=="0"  call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_PARSE% 011 "div0 console_width"
if "%console_height%"=="0" call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_PARSE% 011 "div0 console_height"
...
call :Generate_Environment_Config || (
  call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_IO% 012 "write screen_config.env failed" "file=%screen_cfg_file%"
)
...
call "%RCSU%" -trace INFO ScreenEnv "ok W=%screen_width% H=%screen_height% CW=%console_width% CH=%console_height%"
...
:Generate_Environment_Config
  ...
  > "%_tmp%" (
    echo # auto-generated by ScreenEnvironmentDetection.bat
    echo # date: %date% %time%
    echo.
    echo # Screen resolution [px]
    echo SCREEN_WIDTH=%screen_width%
    echo SCREEN_HEIGHT=%screen_height%
    echo.
    echo # Console size [chars]
    echo CONSOLE_WIDTH=%console_width%
    echo CONSOLE_HEIGHT=%console_height%
    echo.
    echo # Estimated font size [px per char]
    echo CHAR_WIDTH_EST=%char_width_est%
    echo CHAR_HEIGHT_EST=%char_height_est%
    echo.
    echo # Classification
    if %screen_width% LEQ 1366 (
      echo SCREEN_CLASS=SMALL
      echo UI_SCALE=0.8
    ) else if %screen_width% LEQ 1920 (
      echo SCREEN_CLASS=MEDIUM
      echo UI_SCALE=1.0
    ) else (
      echo SCREEN_CLASS=LARGE
      echo UI_SCALE=1.2
    )
    echo.
    echo # Hints
    echo COMPUTER=%COMPUTERNAME%
    echo PROBED_AT=%date% %time%
  ) || exit /b 1
