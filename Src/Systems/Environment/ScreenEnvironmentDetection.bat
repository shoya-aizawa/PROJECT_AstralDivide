...
rem トレースで中身を確認
call "%RCSU%" -trace INFO ScreenEnv "paths cfg=[%_cfg%] dir=[%_screen_dir%] file=[%_screen_file%]"


rem === ログ & ESC ==============================================================
set "LOG_PREFIX=env"
call "%RCSU%" -trace INFO ScreenEnv "start cfg=%_cfg%"
...
call :Generate_Environment_Config || (
  call "%RCSU%" -throw %RCS_S_ERR% %RCS_D_SYS% %RCS_R_IO% 012 "write screen_config.env failed" "file=%_screen_file%"
)
