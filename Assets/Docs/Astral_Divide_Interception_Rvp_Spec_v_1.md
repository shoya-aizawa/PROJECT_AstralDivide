...
> 本書に記載されているインターセプトモード、および `.rvp` / `.ack` ファイルを介した双方向通信、プラグイン注入機能（DDAや動的チュートリアルなど）は現段階では未実装の将来計画（Draft）です。
> 現在の `Watchdog_Host.bat` は死活監視 HUD 機能のみを提供しています。
...
### 💡 設計思想と背景（Design Philosophy & Context）
1. **バッチファイル即時クラッシュ時の可観測性（Visibility）担保**
   Windowsのバッチファイル（`.bat`）は、実行中に致命的な構文エラーやシステム例外が発生すると、`pause`等の待機コマンドを通らない限り、ウィンドウが即座に閉じられてクラッシュする特性を持つ。これにより「何が起きたか」のログすら残らず原因究明が困難になる。  
   このバッチ独自の限界に対処するため、外部監視プロセスである `Watchdog_Host.bat` が `Main.bat` を相互監視（心拍・プロセス生存確認）し、異常終了時にも安全に状況を記録・検知できる堅牢なデバッグ基盤を提供する。
2. **ADMINモードとHTTPサーバーを経由した遠隔診断ビジョン**
   プレリリースやデバッグ検証において、開発者が `ADMIN` モードで待機し、クライアント側の `Watchdog` が `INTERCEPT` モードで稼働している場合、ローカル/リモートの HTTP サーバー（`ad-chat-server.ps1`等）を経由した双方向通信フックを可能にする。これにより、遠隔のプレイヤー環境に対してリアルタイムで「ヒント注入」「変数パッチ」「リモートデバッグ」などの遠隔診断やサポートを行える拡張性を設計の前提としている。
